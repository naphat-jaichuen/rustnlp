...
async fn main() -> Result<()> {
    ...
    let context = CommandContext {
        command: "ask_ai".to_string(),
        task: "ask_ai".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for ask_ai command
        Ok(r#"{"command": "ask_ai", "suggested_commands": ["echo 'Add specific commands for ask ai here'"]}"#.to_string())
    }
}

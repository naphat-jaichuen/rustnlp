...
async fn main() -> Result<()> {
    ...
    info!("Starting get file from handler");
    ...
    let context = CommandContext {
        command: "get_file_from".to_string(),
        task: "get_file_from".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for get_file_from command
        Ok(r#"{"command": "get_file_from", "suggested_commands": ["echo 'Add specific commands for get file from here'"]}"#.to_string())
    }
}

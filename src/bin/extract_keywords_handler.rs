...
async fn main() -> Result<()> {
    ...
    info!("Starting extract keywords handler");
    ...
    let context = CommandContext {
        command: "extract_keywords".to_string(),
        task: "extract_keywords".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for extract_keywords command
        Ok(r#"{"action": "extract_keywords", "result": "Default extract keywords result"}"#.to_string())
    }
}

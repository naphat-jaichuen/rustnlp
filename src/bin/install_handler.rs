...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for install command
        Ok(r#"{"command": "install", "suggested_commands": ["brew install {{package}}", "apt-get install {{package}}", "cargo install {{package}}"]}"#.to_string())
    }
}

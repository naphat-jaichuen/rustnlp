...
async fn main() -> Result<()> {
    ...
    info!("Starting open file handler");
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for open_file command
        Ok(r#"{"command": "open_file", "suggested_commands": ["echo 'Add specific commands for open file here'"]}"#.to_string())
    }
}

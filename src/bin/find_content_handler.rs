...
async fn main() -> Result<()> {
    ...
    info!("Starting find content handler");
    ...
    let context = CommandContext {
        command: "find_content".to_string(),
        task: "find_content".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for find_content command
        Ok(r#"{"command": "find_content", "suggested_commands": ["echo 'Add specific commands for find content here'"]}"#.to_string())
    }
}

...
async fn main() -> Result<()> {
    ...
    info!("Starting open app handler");
    ...
    let context = CommandContext {
        command: "open_app".to_string(),
        task: "open_app".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for open_app command
        Ok(r#"{"command": "open_app", "suggested_commands": ["echo 'Add specific commands for open app here'"]}"#.to_string())
    }
}

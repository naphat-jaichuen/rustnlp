...
async fn main() -> Result<()> {
    ...
    info!("Starting checkout handler");
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for checkout command
        Ok(r#"{"command": "checkout", "suggested_commands": ["echo 'Add specific commands for checkout here'"]}"#.to_string())
    }
}

...
async fn main() -> Result<()> {
    ...
    info!("Starting google search handler");
    ...
    let context = CommandContext {
        command: "google_search".to_string(),
        task: "google_search".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for google_search command
        Ok(r#"{"command": "google_search", "suggested_commands": ["echo 'Add specific commands for google search here'"]}"#.to_string())
    }
}

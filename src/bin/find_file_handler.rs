...
async fn main() -> Result<()> {
    ...
    info!("Starting find file handler");
    ...
    let context = CommandContext {
        command: "find_file".to_string(),
        task: "find_file".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for find_file command
        Ok(r#"{"command": "find_file", "suggested_commands": ["echo 'Add specific commands for find file here'"]}"#.to_string())
    }
}

...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for diff command
        Ok(r#"{"command": "diff", "suggested_commands": ["echo 'Add specific commands for diff here'"]}"#.to_string())
    }
}

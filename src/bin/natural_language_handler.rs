...
async fn main() -> Result<()> {
    ...
    info!("Starting natural language handler");
    ...
    let context = CommandContext {
        command: "natural_language".to_string(),
        task: "natural_language".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for natural_language command
        Ok(r#"{"action": "natural_language", "result": "Default natural language result"}"#.to_string())
    }
}

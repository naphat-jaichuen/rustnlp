...
async fn main() -> Result<()> {
    ...
    info!("Starting show tools handler");
    ...
    let context = CommandContext {
        command: "show_tools".to_string(),
        task: "show_tools".to_string(),
        input_text,
        parsed_result,
        confidence: Some(0.9),
        timestamp: Utc::now(),
...
fn get_parsed_result() -> Result<String> {
    ...
    if let Ok(result) = env::var("PARSED_RESULT") {
        ...
        // Default parsed result for show_tools command
        Ok(r#"{"command": "show_tools", "suggested_commands": ["echo 'Add specific commands for show tools here'"]}"#.to_string())
    }
}
